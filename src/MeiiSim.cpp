...
void MeiiSim::set_enabled(bool _enabled) {
    if (!_enabled) {
        set_torques({0,0,0});
    }
    enabled = _enabled;
...
double MeiiSim::get_position(int i) {
...
double MeiiSim::get_velocity(int i) {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
double MeiiSim::get_acceleration(int i) {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
double MeiiSim::get_torque(int i) {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
void MeiiSim::set_torque(int i, double tau) {
    if (!enabled)
        return;
    ms_in_data[i] = tau;
...
std::vector<double> MeiiSim::get_positions() {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
std::vector<double> MeiiSim::get_velocities() {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
std::vector<double> MeiiSim::get_accelerations() {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
std::vector<double> MeiiSim::get_torques() {
    ms_out_data = ms_out.read_data();
    if (ms_out_data.size() == 12)
...
void MeiiSim::set_torques(const std::vector<double>& Tau) {
    if (!enabled)
        return;
    ms_in.write_data(Tau);
}

void MeiiSim::calibrate() {
    ...
    int calibrating_joint = 0;         // joint currently calibrating
